    ...
    let%lwt () = Workspace.register_eval ~//p
      begin
        fun path properties -> begin
          ...
          let name = Properties.get_or_default "name" ~default:"Yaks OCaml!" properties in

          let%lwt name' = if Astring.is_prefix ~affix:"/" name then
            (
              let%lwt () = Lwt_io.printf "   >> Get name to use from Yaks at path: %s\n" name in
